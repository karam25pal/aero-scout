...
// Full airlines database from dotmarn/Airlines JSON (700+ airlines with logos)
import airlinesData from '@/data/airlines.json';

// Build name → logo lookup from the JSON
const AIRLINE_LOGO_MAP: Record<string, string> = {};
const AIRLINE_ID_MAP: Record<string, string> = {};
for (const airline of airlinesData as Array<{ id: string; name: string; logo: string }>) {
  const key = airline.name.toLowerCase().trim();
  AIRLINE_LOGO_MAP[key] = airline.logo;
  AIRLINE_ID_MAP[key] = airline.id;
}

const getAirlineLogo = (name: string, logoUrl: string): string => {
  const lowerName = name?.toLowerCase().trim() || '';

  // Try full airlines JSON lookup (700+ airlines with Kiwi CDN logos)
  if (AIRLINE_LOGO_MAP[lowerName]) return AIRLINE_LOGO_MAP[lowerName];

  // Try matching by ID if the name itself is a code
  const byId = (airlinesData as Array<{ id: string; logo: string }>).find(
    a => a.id.toLowerCase() === lowerName
  );
